  ...
  const busLink = (() => {
    ...
    const leadFleet = async () => {
      ...
      for (const [name, c] of lead.conns) {
        out.push({ name, online: true, lastSeen: c.lastSeen, sessions: c.sessions, msgCount: c.msgCount || 0, lastSos: c.lastSos || null,
          ...
          ws.on('message', async (raw) => {
            ...
            if (m.t === 'consent-request') {
              ...
              consentStore.requestRemote(m.consent).then(() => {
                ...
                imBridge.send('TermFleet 审批卡', m.consent.requester + ' 请求连接你的会话（' + (m.consent.target || '') + '，30 分钟限时）。打开成员页答复：http://127.0.0.1:3181/dsh-termfleet/app')
              })
