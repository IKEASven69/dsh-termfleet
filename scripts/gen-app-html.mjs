...
.bcard .chead .t{flex:1;min-width:0}
...
function renderBoard(){
  ...
  STATUSES.forEach(function(st){
    ...
    items.forEach(function(t){
      ...
      c.innerHTML = '<div class="chead"><div class="t">'+esc(t.title)+'</div>'
        + '<button class="ibtn stq" data-stmenu="'+t.id+'" title="流转状态">'+ic('ChevronDown',13)+'</button></div>'
        + '<div class="m">'+cardMeta(t)+'</div>';
      c.onclick = function(ev){ if(ev.target.closest('.claim')||ev.target.closest('.stq'))return; openDrawer(t.id); };
      col.appendChild(c);
    });
    el.querySelectorAll('[data-stmenu]').forEach(function(b){
      b.onclick = function(ev){ ev.stopPropagation();
        var t = ST.tasks.filter(function(x){return x.id===b.getAttribute('data-stmenu')})[0];
        openStatMenu(ev, t); };
    });
    el.appendChild(col);
  });
...
function openDrawer(id){
  ...
  // 抽屉头部操作：状态 pill / 认领 / 流转 / 删除
  var sc = t.status==='done'?'g':t.status==='blocked'?'r':t.status==='review'?'a':t.status==='doing'?'b':'';
  document.getElementById('dStatusPill').innerHTML = '<span class="pill '+sc+'">'+stLabel(t.status)+'</span> <span class="prio '+t.prio+'">'+t.prio+'</span>';
  var cb=document.getElementById('dClaimBtn');
  cb.innerHTML = t.owner ? ic('User',13)+' '+esc(t.owner) : ic('User',13)+' 认领';
  cb.disabled = !!t.owner;
  cb.onclick = function(){ act('claim', t.id).then(function(){ var nt=ST.tasks.filter(function(x){return x.id===t.id})[0]; if(nt) openDrawer(nt.id); }); };
  var fb=document.getElementById('dFlowBtn'); fb.innerHTML='状态 '+ic('ChevronDown',12);
  fb.onclick=function(ev){ var cur=ST.tasks.filter(function(x){return x.id===t.id})[0]||t; openStatMenu(ev, cur); cur._after=undefined;
    window.__flowTaskId=t.id; };
